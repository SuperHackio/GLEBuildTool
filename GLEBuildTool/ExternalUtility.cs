...
public static class ExternalUtility
{
    ...
    public static void GenerateExternalsData(string RegionShort, List<GLESymbolDefinition> Externals)
    {
        ...
        GenerateSyatiModule(ExternalsFolder, RegionShort, Externals);
    }

    private static async void GenerateSyatiModule(string ExternalsFolder, string RegionShort, List<GLESymbolDefinition> Externals)
    {
        ...
        // Create the ModuleInfo first
        string ModuleInfoProperties =
            $"""
                "Generated: {DateTime.Now:F}",
                "Symbol Count: {Externals.Count}",
                "GLE Version: {Program.GLEVERSION.Replace('_', '.')}"
            """;
        string ModuleInfo = MODULE_JSON.Replace("\"####\"", ModuleInfoProperties);
        Task t = File.WriteAllTextAsync(Path.Combine(ModuleFolder, "ModuleInfo.json"), ModuleInfo);
            ...
            for (int j = 0; j < Definitions.Count; j++)
            {
                ...
                for (int k = 0; k < Parameters.Length; k++)
                {
                    ...
                    for (int l = 0; l < result.Desc.Count; l++)
                        FunctionDoc.Append(@"      * \param ").Append(result.Name).Append(' ').AppendLine(result.Desc[l]);
        ...
        string HeaderInfo =
            ...
            *  GLE Version: {Program.GLEVERSION.Replace('_','.')}
            """;
        string FinalHeader = string.Format(GLE_HEADER, HeaderData, HeaderInfo);
        Task tt = File.WriteAllTextAsync(Path.Combine(IncludeFolder, "GalaxyLevelEngine.h"), FinalHeader);

        // Prepare the Symbols file!
        List<string> ExternalSymbolList = new(Externals.Count);
        foreach (var externalSymbol in Externals)
            ExternalSymbolList.Add($"{externalSymbol.Name}=0x{externalSymbol.Address:X8}");
        File.WriteAllLines(Path.Combine(SymbolsFolder, $"{REGIONS_SYATI[Array.IndexOf(REGIONS_GLE, RegionShort)]}.txt"), ExternalSymbolList);
